...
void main() {
  ...
  // kicks jolt the tracking band like a bumped VCR
  uv.x += tear * (hash21(vec2(floor(t * 9.0), 1.0)) - 0.3) * (0.05 + u_bass * 0.25 + u_bassHit * 0.4);
  ...
  col = mix(col, vec3(snow), tear * 0.7 + (u_treble * 0.12 + u_trebleHit * 0.25) * snow);
